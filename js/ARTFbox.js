var ARTFbox = function() {
	var _pub = {
		...
		// how fast to fade in the overlay/ARTFbox (this is each step in ms)
		fade_in_speed: 300,
		fade_out_speed: 300,

		// our attribute identifier for our ARTFbox elements
		attribute_name: 'rel',
		...
		/**
		 * Checks a container for specified tags containing rel="ARTFbox"
		...
		checkTags: function(container, tag_name) {
			...
			for (var i=0; i<els.length; i++) {
				if (els[i].getAttribute(_pub.attribute_name)) {
					...
					if ((t.indexOf("ARTFbox") != -1) || t.toLowerCase() == "ARTFbox") { // check if this element is an ARTFbox element
		...
		/**
		 * Sets the content of the ARTFbox
		...
		/**
		 * Empties the content of the ARTFbox (also hides the loading indicator)
		 */
		clear: function() {
		...
		/**
		 * Repositions the ARTFbox wrapper based on the params set originally.
		 */
		reposition: function() {
			if (!active.is_loaded) return;
			...
			// update ARTFbox width/height/position
			if (active.dimensions) {
				...
				// this needs to be height - ARTFbox_content[margin+padding+border]
		...
		/**
		 * Handles the onclick event for ARTFbox anchors.
		 * @param {Event} e
		 */
		handleTag: function(e) {
			...
			if (_pub.inherit_frames && window.parent) window.parent.ARTFbox.showURL(url, title, params);
	...
	/**
	 * Creates the ARTFbox container and appends it to an element
	 * @param {HTMLObject} elem Container to attach to
	 * @return {HTMLObject} ARTFbox element
	 */
	var create = function(elem) {
		pagesize = _pub.getPageSize();
		
		// TODO: why isnt this using DOM tools
		// a trick on just creating an ARTFbox wrapper then doing an innerHTML on our root ARTFbox element
		els.container = document.createElement('div');
		els.container.id = 'ARTFbox';
		...
		els.overlay.id = 'ARTFbox_overlay';
		...
		els.loading.id = 'ARTFbox_loading';
	...
	var drawCSS = function() {
		...
		var core_styles = "#ARTFbox {z-index:1000000;text-align:left;} #ARTFbox_overlay {z-index:1000000;} #ARTFbox_loading {position:absolute;z-index:1000001;} #ARTFbox_wrapper {margin:30px;position:absolute;top:0;left:0;z-index:1000001;} #ARTFbox_content {z-index:1000002;margin:27px 5px 5px 5px;padding:2px;} #ARTFbox_content object {display:block;} #ARTFbox_content .ARTFbox_image {width:100%;height:100%;margin:0;padding:0;border:0;display:block;} #ARTFbox_footer_wrapper a {float:right;display:block;outline:0;margin:0;padding:0;} #ARTFbox_footer_wrapper {text-align:left;position:absolute;top:5px;right:5px;left:5px;white-space:nowrap;overflow:hidden;}";
		
		// Default style/theme/skin/whatever
		var default_skin = "#ARTFbox_footer_wrapper {font-weight:bold;height:20px;line-height:20px;} #ARTFbox_footer_wrapper a {text-decoration:none;background:#888;border:1px solid #666;line-height:16px;padding:0 5px;color:#333;font-weight:bold;font-family:Verdana, Arial, Helvetica, sans-serif;font-size:10px;} #ARTFbox_footer_wrapper a:hover {background-color:#bbb;color:#111;} #ARTFbox_footer_wrapper {font-size:12px;font-family:Verdana, Arial, Helvetica, sans-serif;color:#111;} #ARTFbox_wrapper {border:1px solid #ccc;} #ARTFbox_wrapper {background-color:#999;}#ARTFbox_content {background-color:#eee;border:1px solid #666;} #ARTFbox_loading {padding:50px; background:#000;color:#fff;font-size:16px;font-weight:bold;}";
	...
	var initialize = function() {
		...
		for (var i=0, el=null; (el = els[i]); i++) {
			...
			src = src.split('?')[0];
			if (src.substr(src.length-8) == '/ARTFbox.js') {
	...
	_pub.addEvent(window, 'keypress', function(e){ if (e.keyCode == (window.event ? 27 : e.DOM_VK_ESCAPE)) { ARTFbox.hide(); }});
	 ...
	 */
	var ARTFboxPlugin_Container = function() {
		var was_error = false;
		...
		return {
			...
			/**
			 * Handles the output
			 * @param {ARTFbox} ARTFbox
			 * @param {String} url
			 * @return {ARTFboxContent} an instance or subclass of ARTFboxContent
			 */
	...
	_pub.plugins.register(ARTFboxPlugin_Container, true);
